...
export class UsuariosService {
  ...
  getUserByEmail(email: string): Observable<Usuarios> {
    const params = new HttpParams().set('correo', email);
    return this.http.get<Usuarios>(`${this.apiUrl}/buscar`, { params });
  }

  getProgramsByMsf(msf: string): Observable<Programs[]> {
    return this.http.get<Programs[]>(`${this.apiUrl}/${msf}/programas`);
  }
}
