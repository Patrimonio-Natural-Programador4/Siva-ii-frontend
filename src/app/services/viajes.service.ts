...
export class ViajesService {
  ...
  saveViaje(viaje: Viajes): Observable<any> {
    const payload = {
      ...
      fecha_fin_viaje: viaje.fecha_fin_viaje
  ...
  getHistorialAprobacion(idRegistro: number | string, tipoSolicitud: string): Observable<SolicitudAprobacionHistorial[]> {
    const params = new HttpParams()
      .set('guid', String(idRegistro))
      .set('tipo_solicitud', tipoSolicitud);

    return this.http.get<SolicitudAprobacionHistorial[]>(`${environment.apiUrl2}/solicitudes-aprobacion/historial_aprobacion`, { params });
  }

  getViajesFiltro(page: Number = 1, id_estado: Number[] = [-1], filtro: string, fechaDesde: Date | null, fechaHasta: Date | null, programa: number | null = null): Observable<Viajes[]> {
  ...
  getValidacionAccionesAprobacion(id: string, tipo: string): Observable<ResponseRequest> {
    return this.http.get<ResponseRequest>(`${this.apiUrl}/${id}/validar_acciones_aprobacion?tipo=${tipo}`);
  }

  accionSolicitudAprobacion(guid: string, accion: AccionesSolicitudAprobacion): Observable<ResponseRequest> {
    return this.http.post<ResponseRequest>(`${this.apiUrl}/${guid}/accion_solicitud_aprobacion`, accion);
  }
}
